    ...
    $scope.loginEmail = function(formName,cred) {
      ...
      if(formName.$valid) {  // Check if the form data is valid or not
          ...
          firebase.auth().signInWithEmailAndPassword(cred.email,cred.password).then(function(result) {
            ...
            function(error) {
              sharedUtils.hideLoading();
              sharedUtils.showAlert("Observe", "Erro de autenticação ");
            }
        );

      }else{
        sharedUtils.showAlert("Observe", "Os dados inseridos não são válidos");
      }
    ...
    $scope.signupEmail = function (formName, cred) {
      ...
      if (formName.$valid) {  // Check if the form data is valid or not
        ...
        firebase.auth().createUserWithEmailAndPassword(cred.email, cred.password).then(function (result) {
            ...
            sharedUtils.hideLoading();
            sharedUtils.showAlert("Observe", "Erro de inscrição");
        });
    ...
    $scope.logout=function(){
      ...
      firebase.auth().signOut().then(function() {
        ...
        $state.go('tabsController.login', {}, {location: "replace"});
         ...
         sharedUtils.showAlert("Error","Não foi possível sair")
      });
...
.controller('novoCtrl' , function($scope , $rootScope) { 

    $rootScope.extras=true;

})

.controller('fase1Ctrl' , function($scope , $rootScope) {
...
.controller('settingsCtrl', function($scope,$rootScope,fireBaseData,$firebaseObject,
    ...
    $scope.addManipulation = function(edit_val) {  // Takes care of address add and edit ie Address Manipulator
      ...
      if(edit_val!=null) {
        ...
        var sub_title="Edite seu Endereço";
      }
      else {
        $scope.data = {};    // For adding new address
        var title="Adicionar Endereço";
        var sub_title="Adicione seu Novo Endereço";
      }
      // An elaborate, custom popup
      var addressPopup = $ionicPopup.show({
        template: '<input type="text"   placeholder="Nome"  ng-model="data.nickname"> <br/> ' +
                  '<input type="text"   placeholder="Endereço" ng-model="data.address"> <br/> ' +
                  '<input type="number" placeholder="CEP" ng-model="data.pin"> <br/> ' +
                  '<input type="number" placeholder="Telefone" ng-model="data.phone">',
        title: title,
        subTitle: sub_title,
        scope: $scope,
        buttons: [
    ...
    $scope.deleteAddress = function(del_id) {
      var confirmPopup = $ionicPopup.confirm({
        ...
        template: 'Tem certeza de que deseja excluir este endereço ?',
        buttons: [
    ...
    $scope.save= function (extras,editable) {
      ...
      if(editable.password!="" && editable.password!=null  ){
        ...
        sharedUtils.showAlert("Conta "," Senha atualizada");
      }
...
.controller('detailsCtrl', function($scope,$rootScope) {
...
.controller('checkoutCtrl', function($scope,$rootScope,sharedUtils,$state,$firebaseArray,
    ...
    $scope.payments = [
      ...
      {id: 'NETBANK', name: 'Transferência Bancária'},
      {id: 'COD', name: 'COD'}
    ];
    ...
    $scope.pay=function(address,payment){
      ...
      if(address==null || payment==null){
        ...
        sharedUtils.showAlert("Erro "," Escolha entre os modos de endereço e pagamento.")
      }
      else {
        ...
        sharedUtils.showAlert("Info", "Compra bem sucedida");
    ...
    $scope.addManipulation = function(edit_val) {  // Takes care of address add and edit ie Address Manipulator
      ...
      if(edit_val!=null) {
        ...
        $scope.data = {};    // For adding new address
        var title="Adicionar Endereço";
        var sub_title="Adicionar seu novo Endereço";
      }
      // An elaborate, custom popup
      var addressPopup = $ionicPopup.show({
